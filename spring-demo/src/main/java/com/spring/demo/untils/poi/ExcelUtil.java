...
public class ExcelUtil {
     ...
     * Excel版本号
     */
    private final static String excelVersion = ".xlsx";
    ...
    public ExcelUtil(InputStream is, String version) throws FileNotFoundException, IOException {
        if (excelVersion.contains(version)) {
     ...
     * @throws Exception 异常处理
     */
    public <T> List<T> readValue(int sheetIx, int start, int end, Class<T> clazz) throws Exception {
        ...
        List<Field> feilds = new ArrayList<>();
        List<PoiHelper> poiHelpers = new ArrayList<>();

        for (Field field : declaredFields) {
            PoiHelper annotation = field.getAnnotation(PoiHelper.class);
            if (annotation != null) {
                feilds.add(field);
                poiHelpers.add(annotation);
            }
        }
        for (int i = start; i <= end; i++) {
            assginList(clazz, sheet, list, cols, feilds, poiHelpers, i);
        }
     ...
     * 泛型赋值
     */
    private <T> void assginList(Class<T> clazz, Sheet sheet, List<T> list, int cols, List<Field> feilds, List<PoiHelper> poiHelpers, int i) throws InstantiationException, IllegalAccessException {
        ...
        for (int j = 1; j < cols; j++) {
            ...
            String value = getCellValueToString(row.getCell(j));
            if (!StringUtils.isEmpty(value)) {
                assignValue(feilds.get(j-1), poiHelpers.get(j-1), value, obj);
            } else {
                count++;
            }
        }
     ...
     * value  赋值
     */
    private <T> void assignValue(Field field, PoiHelper poiHelper, String value, T obj) throws IllegalAccessException {
        field.setAccessible(true);
        switch (poiHelper.type()) {
            case 1:
                field.set(obj, value);
                break;
            case 2:
            ...
            case 5:
                ...
                break;
            case 6:
                field.set(obj, LocalDate.parse(value, DateTimeFormatter.ofPattern(this.pattern)));
                break;
            case 7:
     ...
     * @throws IOException IO异常
     */
    public boolean write(int sheetIx, List<List<String>> rowData, int startRow, boolean isSetCellStyle, int type) {
    ...
    public static List<String> getHead(String classPath) throws ClassNotFoundException {
        ...
        for (Field field : fields) {
            Annotation type = field.getAnnotation(PoiHelper.class);
            if (type != null) {
                if (type instanceof PoiHelper) {
                    PoiHelper POIHelper = (PoiHelper) type;
                    list.add(POIHelper.value());
                }
            }
        }
        return list;
    ...
    public <T> List<List<String>> getBodyDate(List<T> list, String className) {
        return getBodyDate(list, className, "");
    }

    public <T> List<List<String>> getBodyDate(List<T> list, String className, String pattern) {
        ...
        try {
            ...
            for (int i = 0; i < list.size(); i++) {
                ...
                template.add(String.valueOf(i+1));
                for (Field field : fields) {
                    Annotation type = field.getAnnotation(PoiHelper.class);
                    ...
                    if (type != null) {
                        if (type instanceof PoiHelper) {
                            ...
                            switch (POIHelper.type()) {
                                ...
                                case 6:
                                    if (field.get(t) != null) {
                                        LocalDate flag = (LocalDate) field.get(t);
                                        String format = flag.format(DateTimeFormatter.ofPattern("yyyy-MM-dd"));
                                        template.add(format);
                                    } else {
                                        //template.add("");
                                    }
                                ...
                                case 7:
                                    if (field.get(t) != null) {
                                        template.add(String.valueOf(field.get(t)));
                                    }
                                    break;
                                default:
                                    break;
